...
def halfcicle(x,y,r):
    ...
    y1=float(y[0])+r*np.sin(tx)*2
    plt.plot(x1,y1,'k-',lw=1.5)
    x2=float(x)+r*np.cos(ty)
    y2=float(y[1])+r*np.sin(ty)*2
...
def loc_new(loc,height,h):
    ...
    return loc_new
def chr_loc(data,height,direction,r,h=0.012):
    for k in data:
        loc_chr=[a[2] for a in k]
        #print(loc_chr)
        loc_chr_new=loc_new(loc_chr,height,h)
        for i in range(len(k)):
            if direction[k[i][0]] == 1:
                plt.plot([float(k[i][1]),float(k[i][1])-0.013],[float(k[i][2]),float(loc_chr_new[i])],lw=0.5,color='black')
                plt.text(float(k[i][1])-0.037,float(loc_chr_new[i]),k[i][0],color='blue',fontsize = 5.5,rotation = 0,**align)
            else:
                plt.plot([float(k[i][1])+2*r,2*r+float(k[i][1])+0.013],[float(k[i][2]),float(loc_chr_new[i])],lw=0.5,color='black')
                plt.text(float(k[i][1])+0.048,float(loc_chr_new[i]),k[i][0],color='blue',fontsize = 5.5,rotation = 0,**align)
        #print(loc_chr,loc_chr_new)
def read_gene_pair(fn):
    f=open(fn)
    data=[]
    for line in f.readlines():
        a=line.strip().split("\t")
...
def plot_gene_pair(data,data_loc,color,r):
    for k in data:
        if len(k)<2:
            continue
        gene1,gene2=dict_loc[k[0]],dict_loc[k[1]]
        if gene1[0]>gene2[0]:
            x=[gene1[0]-r,gene2[0]+r]
        else:
            x=[gene1[0]+r,gene2[0]-r]
        plt.plot(x,[gene1[1],gene2[1]],lw=0.5,color=color,alpha=0.5)
if __name__=="__main__":
    ...
    data_gene_pair=read_gene_pair(sys.argv[3])
    data = set([i for k in data_gene_pair for i in k])
    data_loc,dict_loc=gene_loc(data,dict_gff_1,dict_line_1,height,step_y,r)
    direction={}
    for k in data:
        print(dict_gff_1[k])
        if dict_gff_1[k][0] in ['4']:
            direction[k]=0
        else:
            direction[k]=1
    print(direction)
    plot_gene_pair(data_gene_pair,data_loc,'black',r)
    chr_loc(data_loc,height,direction,r)
    root.set_xlim(0, 1)
    root.set_ylim(0, 1)
    root.set_axis_off()
    plt.savefig("chr_genepair.png", dpi=300) 
